...
async def test_throws_when_raw_content_missing_and_file_path_not_provided(
    ...
    with pytest.raises(ValueError, match="Image extractor requires document.file_path or rawContent"):
...
async def test_wraps_file_read_errors_with_file_path_context(
    ...
    with pytest.raises(RuntimeError, match=f"Failed to read image file at {missing_path}"):
        await extractor.extract(doc)
